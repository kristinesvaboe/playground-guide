...
- Users can add a playground missing from OSM by tapping "Add playground", placing a pin on the map, and filling an optional form (name plus the same detail fields); the new playground shows only to them with a pending indicator until an admin approves it (admin approval of submissions is a later ticket)
