...
| `GET` | `/admin/enrichments` | Lists all unreviewed enrichment submissions. Protected by `X-Admin-Key` header. Response: array of `{ id, playgroundId, playgroundName, equipment, ageSuitability, size, surfaceType, otherEquipment, transportInfo, notes, createdAt }`. |
...
- Users can add or edit playground details (age suitability, equipment, other equipment, size, surface type, transport info, notes — all optional, but at least one is required) via a mobile-first bottom-sheet form; submissions are held for review and only visible to their author until approved
